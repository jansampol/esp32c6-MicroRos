...
{
    printf("SPI0 FINAL INTEGRATION TEST\n");
    ...
    ESP_LOGI("TEST", "SPI pins: MOSI=%d MISO=%d CLK=%d",
             SPI0_MOSI_PIN, SPI0_MISO_PIN, SPI0_CLK_PIN);
    ESP_LOGI("TEST", "Main valve bit mask: 0x%04X", MAIN_VALVE_VALVE);
    ESP_LOGI("TEST", "Emergency pin: GPIO %d", EMERGENCY_PIN);

    ESP_LOGI("TEST", "=== Initial MCP register dump ===");
    ...
    ESP_LOGI("TEST", "=== End MCP register dump ===");

    // Start from known state
    spi0.writeMainValve(0x0000);
    spi0.writeLeds(0x00);
    vTaskDelay(pdMS_TO_TICKS(200));

    uint8_t ledPattern = 0x01;
    bool valveOpen = false;
    int lastEmergencyLevel = gpio_get_level(static_cast<gpio_num_t>(EMERGENCY_PIN));
    ...
    while (true) {
        ...
        uint16_t pot0 = spi0.readPotmeters(false);
        uint16_t pot1 = spi0.readPotmeters(true);

        int emergencyLevel = gpio_get_level(static_cast<gpio_num_t>(EMERGENCY_PIN));
        if (emergencyLevel != lastEmergencyLevel) {
            ESP_LOGI("TEST", "Emergency button changed: %d -> %d",
                     lastEmergencyLevel, emergencyLevel);
            lastEmergencyLevel = emergencyLevel;
        }

        uint8_t ledWritten = ledPattern;
        spi0.writeLeds(ledWritten);
        ledPattern <<= 1;
        if (ledPattern == 0 || ledPattern > 0x20) {
            ledPattern = 0x01;
        }
        ...
        ESP_LOGI("TEST",
                 "HB=0x%04X VB=0x%04X SW=0x%02X P0=%4u P1=%4u EM=%d LED=0x%02X VALVE=%s",
                 hButtons,
                 vButtons,
                 switches,
                 pot0,
